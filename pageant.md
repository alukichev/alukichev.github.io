...
## Why Pageant
While Microsoft’s port of OpenSSH has an agent, their idea of how to use it may not be what you really want if your keys are passphrase-protected. After initial decryption with a passphrase, MS `ssh-agent.exe` re-encrypts the key using your user session credentials and stores it permanently in System Registry (`HKCU\Software\OpenSSH\Agent\Keys`). Thereafter (even after a reboot), the key is accessible without a passphrase by any process that talks to the agent in your user session.

That design looks convenient, but it removes the essential detail that passphrase adds to security of your keys: they become available only when the user decides. With MS SSH agent, your keys become slightly more safe than if they were in plain text in some restricted folder: they cannot be stolen but can be used by any software in a user session at any moment. [Here](https://github.com/PowerShell/Win32-OpenSSH/issues/1487#issuecomment-547798640) is a more detailed explanation of the issue and the vendor's ~~prompt reaction~~.
